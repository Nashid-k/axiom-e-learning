...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(MayaChatViewProvider.viewType, provider));
    // Register Inline Autocomplete (Ghost Text)
    const inlineCompletionProvider = vscode.languages.registerInlineCompletionItemProvider({ pattern: '**' }, new MayaInlineCompletionProvider());
    ...
    const addContext = vscode.commands.registerCommand('axiom.addContext', async (uri) => {
        let fileUri = uri;
    ...
    const applyEdit = vscode.commands.registerCommand('axiom.applyEdit', async (options) => {
        try {
...
function deactivate() { }
class MayaChatViewProvider {
    constructor(_extensionUri) {
        this._extensionUri = _extensionUri;
        this._chatHistory = [];
        this._permissionResolver = null;
        this._alwaysAllowMode = false;
    }
    ...
    handleToolPermission(allow, alwaysAllow) {
        if (alwaysAllow)
            this._alwaysAllowMode = true;
        if (this._permissionResolver) {
            this._permissionResolver({ allow, alwaysAllow });
            this._permissionResolver = null;
        }
    }
    async requestToolPermission(toolName, path) {
        if (this._alwaysAllowMode)
            return true;
    ...
    async handleSendMessage(userMessage, codeContext) {
        ...
        const baseUrl = config.get('apiUrl') || 'https://axiom-learn.com';
...
- Always use absolute paths.`;
        ...
        while (agentLoopActive) {
            try {
                ...
                if (accessToken)
                    headers['x-agent-access-token'] = accessToken;
                    ...
                    for (const tMatch of toolCalls) {
                        ...
                        try {
                            ...
                            if (toolName === 'read_file') {
                                ...
                                if (action === 'status')
                                    toolOutput = (await execAsync('git status', { cwd: workspaceUri.fsPath })).stdout;
                                else if (action === 'diff')
                                    toolOutput = (await execAsync('git diff', { cwd: workspaceUri.fsPath })).stdout;
                            ...
                            if (toolOutput.length > 30000)
                                toolOutput = toolOutput.substring(0, 30000) + "\n...[TRUNCATED]";
                            currentMessages.push({ role: 'user', content: `<tool_result>\n<name>${toolName}</name>\n<content>\n${toolOutput}\n</content>\n</tool_result>` });
...
MayaChatViewProvider.viewType = 'axiom.chatView';
class MayaInlineCompletionProvider {
    constructor() {
