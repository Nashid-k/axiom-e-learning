...
export default function VitalsPage() {
    ...
    return (
        <main className="min-h-screen p-6 md:p-12 bg-[var(--surface-base)] text-[var(--fg-primary)]">
            <div className="max-w-7xl mx-auto space-y-12">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-[var(--surface-border)] pb-8">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">System Vitals</h1>
                        <p className="text-sm text-[var(--fg-muted)] mt-1 uppercase tracking-widest font-bold">
                            {snapshot ? `Snapshot: ${formatTs(snapshot.generatedAt)}` : 'Initializing...'}
                        </p>
                    </div>
                    <div className="flex gap-4">
                        <select className="bg-transparent border border-[var(--surface-border)] rounded-md px-3 py-1.5 text-xs font-bold uppercase tracking-widest outline-none text-[var(--fg-secondary)]" value={refreshMs} onChange={(e) => setRefreshMs(Number(e.target.value))}>
                            <option value={1000}>1s Refresh</option>
                            <option value={2000}>2s Refresh</option>
                            <option value={5000}>5s Refresh</option>
                        </select>
                        <select className="bg-transparent border border-[var(--surface-border)] rounded-md px-3 py-1.5 text-xs font-bold uppercase tracking-widest outline-none text-[var(--fg-secondary)]" value={windowHours} onChange={(e) => setWindowHours(Number(e.target.value))}>
                    ...
                    <>
                        ...
                        <VitalsTable title="API Endpoints" headers={['Method', 'Path', 'Reqs', 'Err%', 'Avg', 'P95', 'Status']}>
                            {snapshot.apiSummary.map((row, i) => (
                                <tr key={i} className="border-b border-[var(--surface-border)] last:border-0 hover:bg-[var(--surface-raised)] transition-colors">
                                    ...
                                    <td className={`p-4 text-xs font-bold ${row.errorRate > 0 ? 'text-[var(--color-accent)]' : 'text-[var(--fg-muted)]'}`}>{row.errorRate}%</td>
...
function StatCard({ label, value }: { label: string; value: number }) {
    return (
        <div className="p-6 border border-[var(--surface-border)] rounded-md bg-[var(--surface-base)]">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--fg-muted)] mb-2">{label}</p>
...
function VitalsTable({ title, headers, children }: { title: string; headers: string[]; children: React.ReactNode }) {
    return (
        <section>
            <h2 className="text-sm font-bold uppercase tracking-widest text-[var(--fg-muted)] mb-6">{title}</h2>
            <div className="border border-[var(--surface-border)] rounded-md overflow-hidden overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead className="bg-[var(--surface-raised)] border-b border-[var(--surface-border)]">
                        <tr>
                            {headers.map(h => (
                                <th key={h} className="p-4 text-[10px] font-bold uppercase tracking-widest text-[var(--fg-muted)]">{h}</th>
                            ))}
                        </tr>
                    </thead>
