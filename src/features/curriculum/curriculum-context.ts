...
async function findAdjacentTopics(
  category: Category,
  targetPhase?: number,
  budget: number = 800
): Promise<string> {
...
export async function buildCurriculumContextForTopic(
  category: Category,
  topic: string,
  phase?: number,
  ...
  if (phase && phase > 1 && usedChars < maxChars * 0.7) {
    const prereqs = await findPrerequisites(category, phase);
    if (prereqs) {
  ...
  if (phase && usedChars < maxChars * 0.8) {
    const adjacent = await findAdjacentTopics(category, phase, maxChars - usedChars);
    if (adjacent) {
  ...
  if (phase && usedChars < maxChars) {
    const followups = await findFollowUps(category, phase);
    if (followups) {
...
export async function buildEnhancedCategoryContext(
  ...
  if (topic && phase) {
    return await buildCurriculumContextForTopic(
