...
export default function ResourceModal({ topicTitle, curriculum, phase, onClose }: ResourceModalProps) {
    ...
    return (
        <ModalShell isOpen onClose={onClose} containerClassName="w-[95vw] max-w-4xl max-h-[90vh]">
            <div className="sticky top-0 z-10 bg-white dark:bg-black border-b border-neutral-200 dark:border-neutral-800 p-6 flex justify-between items-start">
                <div>
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {videos.map((video, idx) => (
                                        <a key={idx} href={video.url} target="_blank" rel="noopener noreferrer" className="group border border-neutral-200 dark:border-neutral-800 rounded-md overflow-hidden bg-white dark:bg-black hover:border-neutral-400">
                                            {video.thumbnail && (
                                            ...
                                            <div className="p-4">
                                                <h4 className="font-bold text-sm line-clamp-2 mb-2 group-hover:text-brand-500">{video.title}</h4>
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {articles.map((article, idx) => (
                                        <a key={idx} href={article.url} target="_blank" rel="noopener noreferrer" className="p-4 border border-neutral-200 dark:border-neutral-800 rounded-md hover:border-neutral-400 group">
                                            <div className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-2">{article.source}</div>
                                            <h4 className="font-bold text-sm group-hover:text-brand-500">{article.title}</h4>
                                        </a>
                                    ))}
                                </div>
                            </section>
