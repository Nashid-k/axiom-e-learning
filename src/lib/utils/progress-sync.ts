...
export async function migrateLegacyData(data: ProgressData): Promise<{ migrated: ProgressData; hasChanges: boolean }> {
    const migrated: ProgressData = { ...data };
    let hasChanges = false;

    for (const entry of Object.values(CURRICULUM_REGISTRY)) {
        try {
